# Examples of Aethergraph Usage

This page is your guided tour to **AetherGraph (AG)** through runnable, copy‑pasteable examples. Start with the **Hero Demos** to experience a complete flow in minutes (console + LLM only). Then explore two complementary pattern pools:

* **Method Showcase** — small, focused snippets for individual `context.*` capabilities and services.
* **Concrete Example Patterns** — larger recipes that combine multiple features for real‑world tasks.

> This catalog **evolves over time**. We periodically add new demos, refine existing ones, and may adjust paths as the repo grows. The tables below always reflect the current recommended entry points.

All the examples are in a separate **example [repo](https://github.com/AIperture/aethergraph-examples)**. Please clone the repo and set up `.env` to run the examples.
...
|  4 | **Extend Prompt Services**           | Agents use `context.prompt_store()` to fetch prompts and `context.llm_observer()` to log calls—clean agent code, centralized control.       | `servdemo_examplesices/4_external_service_prompt_store.py`   |
